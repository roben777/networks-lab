...
\date{\today\\ version 1.0}  %   Use current date. 
...
this lab manual is to install a network emulator. The emulator is
...
home computers (optional step) and will configure their school
account to use \kathara \cite{kathara-wiki}. Please note that the school network has \kathara\ already installed. More information
including a few examples of commands are provided in Section
\ref{examples.lab0}. The following sequence of operations is
suggested: 

\begin{enumerate}[(1)]
\item If only configuring an already installed system, go to step
  \ref{config-step}. You will need root access to the computer you are
  installing \kathara\ on. Please do not execute this step on the
  school network.

Download the installation files for \kathara\ from
...
emulator. Unpack both package files into a system folder, such as
\verb$/opt/kathara$. 
...
\item\label{config-step} Define the environment variable
  \verb$NETKIT_HOME$ to point to the \verb$bin$ folder under \kathara's
  installation directory (step 6 in the Linux installation
  instructions). 
\end{enumerate}


\subsection{Creating a first network}
...
\item Change your current directory in the folder where you unpacked
  ...
  \item Depending on the options passed when you started the lab, you
    ...
    information on both hosts. Run \verb$pping$ to check for
...
\item[Stop a running \kathara\ lab:] Change current directory to the
...
\item[Start a \kathara\ lab without opening terminals for each virtual
  ...
  this on your personal computer.

  When the number of virtual machines is large, having a terminal
  window open for each machine is annoying, In addition, you most
  likely need to work from within only a handful of virtual
  machines. Start the lab using the \verb$--noterminals$ option. Then
